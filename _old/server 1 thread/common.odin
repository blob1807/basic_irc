...
load_config :: proc(buf: []u8, s: ^Server, alloc := context.allocator) -> (err: Error) {
    ...
    s.ping_timer.duration = c.timers.ping
    s.ping_check_timer.duration = c.timers.ping_check
...
update_timer :: proc(t: ^Timer) -> bool {
    c := time.tick_now()
...
start_timer :: proc(t: ^Timer) {
    t.left = t.duration
    t.last = time.tick_now()
}

stop_timer :: proc(t: ^Timer) {
    t.left = 0
}

timer_ended :: proc(t: Timer) -> bool {
    return t.left <= 0
}


// WARNING: MODIFIES INPUT STRING!!! Used for nick & usernames
to_lower :: proc(str: string) {
