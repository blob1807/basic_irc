...
init_server :: proc(s: ^Server, addr: string, name := DEFAULT_NAME, network := DEFAULT_NETWORK, set_i_support := true) -> (err: Error) {
    ...
    s.ping_timer.duration = PING_TIMER_DURATION
    s.ping_check_timer.duration = PING_CHECK_TIMER_DURATION
...
server_runner :: proc(s: ^Server) -> (err: Error) {
...

    {
        ...
        c_ptr^ = Channel {
            ...
            to_send_alloc = context.temp_allocator, // TODO: Remove when threading
        }
    ...
    for !sync.atomic_load(&s.close_server) {
        ...
        if .Pinging not_in s.flags {
            if update_timer(&s.ping_timer) {
                reset_timer(&s.ping_check_timer)   
                s.flags += {.Pinging}
            }

        } else {
            if false && update_timer(&s.ping_check_timer) {
                reset_timer(&s.ping_timer)
                s.flags -= {.Pinging}
            } 
        }
...
open_new_clients_thread :: proc(s: ^Server) {
    ...
    for !sync.atomic_load(&s.close_new_client_thread) {
        ...
        start_barrier: sync.Barrier
        sync.barrier_init(&start_barrier, 2)
        c_ptr.thread = thread.create_and_start_with_poly_data3(s, c_pre, &start_barrier, client_thread)
        sync.barrier_wait(&start_barrier)
        */
...
client_thread :: proc(s: ^Server, c: ^Client, _start_barrier: ^sync.Barrier) {
    ...
    defer {
        sync.atomic_add(&c.flags, {.Quit})
        sync.atomic_add(&c.thread_flags, {.Has_Closed})
    }

    temp_arena: virtual.Arena
    arena_error := virtual.arena_init_growing(&temp_arena)
    if arena_error != nil {
        log.fatalf("Failed to create temporary arena in Client Thread(%v) because of: %v", c.user, arena_error)
        return
    }
    ...
    for !sync.atomic_load(&s.close_client_threads) {
        ...
        mess_loop: for sync.atomic_load(&c.flags) & {.Close, .Quit} == {} {
            ...
            case IRC_Errors:
                ...
                if .Errored in c.flags {
                    sync.atomic_add(&c.flags, {.Close})
                }
                sync.atomic_add(&c.flags, {.Errored})
...
channel_thread :: proc(s: ^Server, c: ^Channel, _start_barrier: ^sync.Barrier) {
    ...
    arena_error := virtual.arena_init_growing(&temp_arena)
    if arena_error != nil {
        log.fatalf("Failed to create temporary arena in Channel Thread(%v) because of: %v", c.name, arena_error)
        return
    }
